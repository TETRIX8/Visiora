...
export default function UserProfileButton({ onLoginClick }) {
  ...
  React.useEffect(() => {
    console.log('UserProfileButton credits:', credits);
    console.log('UserProfileButton user:', user);
      ...
      <div className="flex items-center gap-3">
        <div className="flex flex-col items-start mr-2">
          <div className="flex items-center justify-center">
            ...
            <span className="text-xs font-medium">Free: {isLoadingCredits ? '...' : (credits?.free || credits?.freeCredits || 0)}</span>
          </div>
          <div className="flex items-center justify-center">
            ...
            <span className="text-xs font-medium">Paid: {isLoadingCredits ? '...' : (credits?.paid || credits?.paidCredits || 0)}</span>
          </div>
        </div>
        <Button
        ...
        <div className="absolute right-0 mt-2 w-48 rounded-md bg-white dark:bg-slate-900 shadow-lg ring-1 ring-black/5 dark:ring-white/10 focus:outline-none z-10">
          ...
          <div className="p-3 border-b border-gray-200 dark:border-gray-700">
                ...
                <div className="flex items-center">
                  ...
                  <p className="text-md font-bold text-purple-600 dark:text-purple-400">
                    {isLoadingCredits ? '...' : (credits?.total || 
                      (typeof credits === 'object' ? (credits.free + credits.paid) : credits) || 
                      0)}
                  </p>
                </div>
              </div>
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              <p>Free: {credits?.free || credits?.freeCredits || 0} (resets daily)</p>
              <p>Paid: {credits?.paid || credits?.paidCredits || 0}</p>
            </div>
          </div>
          <div className="py-1">
            <button
